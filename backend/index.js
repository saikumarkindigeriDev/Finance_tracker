...
app.get('/api/barchart/',(req,res)=>{
  ...
  const sqsl='select amount,date from transactions where  user_id=? AND  type=?'

const   values=[param1,param2]



  db.query(sqsl,values,(err,results)=>{
    if (err){
      console.log("failed");
...
app.get('/api/getExpense/',(req,res)=>{
 ...
 let values=[param1]
  ...
  if (param2!=="all"){
    ...
    sql=' select * from transactions where  user_id=? AND  type=?'
